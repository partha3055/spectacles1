...
<head>
    ...
    <title>{{ $brand->name }} - Spectacles</title>
...
<body class="bg-gray-50">
    ...
    <div class="container mx-auto px-4 py-12">
        <nav class="text-sm text-gray-500 mb-6">
            ...
            <a href="/brands" class="hover:text-yellow-500">Brands</a>
            <span class="mx-2">/</span>
            <span class="text-gray-800">{{ $brand->name }}</span>
        </nav>
        
        <h1 class="text-4xl font-bold text-gray-900 mb-8">{{ $brand->name }}</h1>
        
        @if($brand->description)
            <p class="text-gray-600 mb-8">{{ $brand->description }}</p>
        @endif
        
        @php $activeProducts = $brand->products->where('is_active', true) @endphp
